...
const PATH : &'static str = "src/adder_v2/project/a04_uf_76/excel/base00.txt";
...
fn test_cdl() {
    let adder = adder();
    let txt = adder.to_cdl("ADDER_UF_31_L7_PN_V00", Process::N3E);
    use std::fs::File;
    use std::io::prelude::*;
...
fn test_netlist() {
    ...
    let txt = adder.to_netlist("adder_uf_31_l7_pn_v00", Process::N3E, decr_info);
    use std::fs::File;
    use std::io::prelude::*;
    let content = "This is the content to write to the file.";
    // 创建一个新文件，如果文件已存在，则覆盖
