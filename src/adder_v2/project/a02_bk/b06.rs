...
const PATH : &'static str = "src/adder_v2/project/a02_bk/excel/b06.txt";
...
fn test_cdl() {
    let adder = adder();
    let txt = adder.to_cdl("BK_ADDER_PP_B06", Process::N3E);
    use std::fs::File;
    use std::io::prelude::*;
...
fn test_netlist() {
    ...
    let txt = adder.to_netlist("adder_bkfast_31_l8_2p_pp", Process::N3E, decr_info);
    use std::fs::File;
    use std::io::prelude::*;
    let content = "This is the content to write to the file.";
    // 创建一个新文件，如果文件已存在，则覆盖
